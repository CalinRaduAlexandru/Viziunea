...
function homeView() {
  ...
  const stepNav=`<nav class="step-nav">${progress()}${step < 4 ? '<button class="round nav-next" data-next aria-label="Pasul următor">→</button>' : ''}</nav>`;
  return `<main class="onboarding ${coverScreen?'cover-onboarding':''}">${coverScreen?'':stepLabel}${content}${coverScreen?'':stepNav}</main>`;
}
